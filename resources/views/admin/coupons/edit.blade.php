        ...
        <div class="max-w-3xl mx-auto">
            ...
            <div class="mb-6">
                <a href="{{ route('admin.coupons.index') }}"
                    class="text-gray-600 hover:text-pink-500 text-sm flex items-center mb-2">
                ...
                <p class="mt-1 text-sm text-gray-600">Modifica el código de descuento <span
                        class="font-mono font-bold text-pink-600">{{ $coupon->code }}</span></p>
            </div>

            {{-- Stats --}}
            <div class="bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl p-4 mb-6 text-white">
            ...
            <form action="{{ route('admin.coupons.update', $coupon) }}" method="POST"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="code" id="code" required
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-pink-500 uppercase font-mono"
                ...
                {{-- Espacio (opcional) --}}
                <div>
                    <label for="spaces_id" class="block text-sm font-medium text-gray-700 mb-1">Espacio (opcional)</label>
                    <select name="spaces_id" id="spaces_id"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-pink-500">
                        <option value="">Global (todos los espacios)</option>
                        @foreach($spaces as $space)
                            <option value="{{ $space->id }}" {{ old('spaces_id', $coupon->spaces_id) == $space->id ? 'selected' : '' }}>{{ $space->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex justify-end gap-4 pt-6 border-t border-gray-200">
                    ...
                    <button type="submit"
                        class="px-6 py-3 bg-gradient-to-r from-pink-500 to-pink-600 text-white rounded-lg hover:from-pink-600 hover:to-pink-700 transition-all shadow-lg">
                        Guardar Cambios
                    </button>
                </div>
            </form>
