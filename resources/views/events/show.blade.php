...
@section('content')
    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 relative">
        ...
        <div
            class="fixed inset-0 bg-gradient-to-br from-pink-50/30 via-purple-50/20 to-blue-50/30 backdrop-blur-[2px] pointer-events-none">
        </div>
        <div class="relative overflow-hidden">
            @if($event->banner)
                {{-- Banner estilo Facebook Cover: aspect ratio 16:9 que mantiene la proporción --}}
                <div class="relative w-full overflow-hidden bg-gray-900" style="aspect-ratio: 16/9; max-height: 70vh;">
                    <img src="{{ \App\Helpers\ImageHelper::getImageUrl($event->banner) }}" alt="{{ $event->name }}"
                        id="hero-image" class="absolute inset-0 w-full h-full object-cover object-center">

                    <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent"></div>

                    <div class="absolute inset-0 flex items-end justify-center pb-8 sm:pb-12 lg:pb-16">
                        <div class="text-center text-white px-4 sm:px-6 max-w-4xl w-full">
                            <div
                                class="backdrop-blur-lg bg-black/30 rounded-2xl p-4 sm:p-6 lg:p-8 border border-white/20 shadow-2xl">
                                <h1 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-light mb-3 tracking-wide">
                                    {{ $event->name }}
                                </h1>
                                <div class="flex flex-wrap items-center justify-center gap-4 sm:gap-6 mb-4">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span
                                            class="text-sm font-medium text-white/90">{{ \Carbon\Carbon::parse($event->date)->format('d M Y \a \l\a\s H:i') }}</span>
                                    </div>
                                </div>
                                @if($event->ticketTypes->count() > 0)
                                    <div class="inline-block ">
                                        ...
                                        <p class="text-white text-lg font-semibold">
                                            ${{ number_format($event->ticketTypes->min('pivot.price'), 2) }}</p>
                                    </div>
                                @endif
                            </div>
                        </div>
            ...
            @else
                <div
                    class="h-[60vh] min-h-[500px] bg-gradient-to-br from-blue-500 via-purple-600 to-indigo-700 flex items-center justify-center relative">
        ...
        <div
            class="relative z-10 -mt-16 sm:-mt-24 lg:-mt-32 max-w-4xl mx-auto px-2 sm:px-4 lg:px-6 py-6 sm:py-8 lg:py-12 space-y-4 sm:space-y-6 lg:space-y-8">

            <div
                class="bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 border border-white/30 shadow-xl">
                ...
                @if($event->agenda && $event->agenda !== 'N/A')
                    <div class="mb-8">
                        ...
                        <div
                            class="bg-gradient-to-br from-white/70 via-purple-50/40 to-pink-50/30 backdrop-blur-md rounded-xl p-4 sm:p-6 border border-purple-100/50 shadow-lg overflow-hidden">
                            <div
                                ...
                                {!! \Illuminate\Support\Str::markdown($event->agenda, ['html_input' => 'strip', 'allow_unsafe_links' => false]) !!}
                            </div>
                        </div>
                    </div>
                @endif
                ...
                @endif
            {{-- Tags y Keywords - Sección compacta al final --}}
                    @if(($event->tags && $event->tags->count() > 0) || ($event->space && $event->space->keywords))
                        <div class="mt-6 pt-4 border-t border-gray-200/50">
                            @if($event->tags && $event->tags->count() > 0)
                                <div class="mb-3">
                                    <div class="flex flex-wrap items-center gap-1.5">
                                        <svg class="w-3.5 h-3.5 text-pink-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z">
                                            </path>
                                        </svg>
                                        @foreach($event->tags as $tag)
                                            <a href="{{ config('app.url') }}/search?tag={{ $tag->id }}"
                                                class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-pink-50 text-pink-700 border border-pink-200 hover:bg-pink-100 transition-colors duration-200">
                                                {{ $tag->name }}
                                            </a>
                                        @endforeach
                                    </div>
        ...
        @push('scripts')
            <script>
                        ...
                        function showNotification(message, type = 'info') {
                            ...
                            notification.className = `fixed top-4 left-4 right-4 sm:left-auto sm:right-4 sm:max-w-sm ${bgColor} text-white px-4 sm:px-6 py-3 rounded-lg shadow-lg z-50 transform translate-y-[-100px] opacity-0 transition-all duration-300`;
