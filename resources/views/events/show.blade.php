                        ...
                        @foreach($event->ticketTypes as $ticketType)
                            ...
                            <div class="bg-white/60 backdrop-blur-sm border-2 border-gray-200 rounded-xl p-6 hover:border-indigo-300 transition-all duration-300 hover:shadow-lg hover:scale-105 flex justify-between">
                                ...
                                <div class="flex items-center justify-center space-x-4 align-middle">
                                    ...
                                    {{-- Input cantidad --}}
                                    <input type="number" id="quantity_{{ $ticketType->id }}" name="tickets[{{ $loop->index }}][quantity]" value="0" min="0" max="{{ $disponibles }}" @if($disponibles == 0) disabled @endif class="w-16 h-12 text-center text-xl font-bold border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200" onchange="updateTotal()">
                                    {{-- Botón + --}}
