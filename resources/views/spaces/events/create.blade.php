...
@section('content')
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="bg-gradient-to-r from-pink-40 to-pink-50 rounded-xl p-6">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                <!-- IMAGEN APP MÓVIL (9:16) -->
                                <div class="bg-white rounded-xl p-4 shadow-sm border-2 border-gray-100">
                                    <label for="image" class="block text-sm font-semibold text-gray-800 mb-2">
                                        Imagen para App Móvil
                                    </label>
                                    <p class="text-xs text-gray-500 mb-3">Proporción 9:16 (1080 × 1920 px) — Vertical para
                                        app</p>
                                    ...
                                    @if(isset($event) && $event->image)
                                        <div class="mt-3 p-3 bg-green-50 rounded-lg border border-green-200">
                                            ...
                                            <img src="{{ \App\Helpers\ImageHelper::getImageUrl($event->image) }}"
                                                alt="Imagen actual"
                                                class="h-40 w-auto object-cover rounded-lg shadow-sm mx-auto">
                                            <p class="text-xs text-gray-500 mt-2 text-center">Selecciona una nueva imagen para
                                                reemplazarla
                                            </p>
                                        </div>
                                    @endif
                                    ...
                                    <!-- Vista Previa Contextual - 9:16 -->
                                    <div class="mt-4">
                                        <p class="text-xs text-gray-500 mb-2 font-medium">Vista previa (proporción 9:16):
                                        </p>
                                        <div class="bg-gray-100 rounded-xl p-3 flex justify-center">
                                            <div class="bg-white rounded-lg shadow-md overflow-hidden w-32">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            // NOTA: EasyMDE se inicializa en el segundo bloque de script más abajo
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const commonConfig = {
                spellChecker: false, // Desactivar corrector automático (suele marcar todo en español)
                nativeSpellcheck: true, // Usar el corrector del navegador
            ...
            // Inicializar EasyMDE para Descripción
            const descriptionEditor = new EasyMDE({
                ...commonConfig,
                element: document.getElementById('description'),
                autosave: {
                    enabled: true,
                    uniqueId: "event_description_{{ $space->id }}_{{ isset($event) ? $event->id : 'new' }}",
                    delay: 1000,
                },
            ...
            // Inicializar EasyMDE para Temario
            const agendaEditor = new EasyMDE({
                ...commonConfig,
                element: document.getElementById('agenda'),
                autosave: {
                    enabled: true,
                    uniqueId: "event_agenda_{{ $space->id }}_{{ isset($event) ? $event->id : 'new' }}",
                    delay: 1000,
                },
                placeholder: "Detalla el cronograma o actividades del evento...",
                minHeight: "300px",
            });
            ...
            document.querySelector('form').addEventListener('submit', function (e) {
                ...
                // Limpiamos las claves de localStorage que EasyMDE usa para autosave
                localStorage.removeItem('smde_' + descUniqueId);
                localStorage.removeItem('smde_' + agendaUniqueId);

                // También limpiamos los IDs anteriores de EasyMDE (primera instancia)
                const descUniqueIdOld = "description_{{ $space->id ?? 'new' }}";
                const agendaUniqueIdOld = "agenda_{{ $space->id ?? 'new' }}";
                localStorage.removeItem('smde_' + descUniqueIdOld);
                localStorage.removeItem('smde_' + agendaUniqueIdOld);
            });
        });
    </script>
