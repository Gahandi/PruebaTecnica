        ...
        @if(empty($cart))
            ...
            <div class="text-center py-12">
                <div class="w-24 h-24 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
                    <svg class="w-14 h-14 transition-transform group-hover:scale-110" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                                    ...
                                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
                                        ...
                                        <div class="flex items-start gap-3 sm:gap-4 w-full sm:w-auto">
                                            @if(isset($item['event_image']) && $item['event_image'])
                                                <img src="{{ \App\Helpers\ImageHelper::getImageUrl($item['event_image']) }}"
                                                    alt="{{ $item['event_name'] ?? 'Evento' }}"
                                                    class="w-16 h-16 sm:w-20 sm:h-20 rounded-lg object-cover flex-shrink-0">
                                            @else
                                                <div
                                                    class="w-16 h-16 sm:w-20 sm:h-20 bg-gray-200 rounded-lg flex items-center justify-center flex-shrink-0">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <div class="mb-2">
                                                    <h3 class="text-base sm:text-lg font-medium text-gray-900">
                                                        {{ $item['event_name'] ?? 'Evento' }}</h3>
                                                    @if(isset($item['event_date']))
                                                        <p class="text-xs sm:text-sm text-gray-500">
                                                            {{ \Carbon\Carbon::parse($item['event_date'])->format('d M Y, H:i') }}</p>
                                                    @endif
                                                </div>
                                                ...
                                                <div class="mb-2 sm:mb-3">
                                                    <h4 class="text-sm sm:text-base font-semibold text-gray-800">
                                                        {{ $item['ticket_type_name'] ?? 'Tipo de Boleto' }}</h4>
                                                    <div class="mt-1">
                                                        <span
                                        ...
                                        <!-- Price and Controls -->
                                        <div
                                            class="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full sm:w-auto gap-3 sm:gap-4">
                                            <div class="text-left sm:text-right">
                                                @if(isset($item['price']) && isset($item['quantity']))
                                                    <p class="text-base sm:text-lg font-semibold text-gray-900">
                                                        ${{ number_format($item['price'], 2) }}</p>
                                                    <p class="text-xs sm:text-sm text-gray-500">por boleto</p>
                                                    <p class="text-sm font-medium text-gray-700">Total:
                                            ...
                                            <div class="flex items-center space-x-2">
                                                <form method="POST" action="{{ route('checkout.update-cart') }}" class="inline"
                                                    ...
                                                    <input type="number" name="quantity" value="{{ $item['quantity'] ?? 0 }}"
                                                        min="0" max="10"
                    ...
                    <div class="bg-white shadow-lg rounded-lg overflow-hidden sticky top-20">
                        ...
                        <div class="p-4 sm:p-6">
                            ...
                            <div class="flex justify-between mb-4">
                                <span class="text-gray-600">{{ $serviceChargeName }} ({{ $serviceChargePercentage }}%)</span>
